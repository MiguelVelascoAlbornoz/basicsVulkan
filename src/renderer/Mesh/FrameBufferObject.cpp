...
void FrameBufferObject::changeResolution(int newWidth, int newHeight) {
    ...
    width = newWidth;
    height = newHeight;
    recreateResources();          // recrea imágenes (mismo sample count, mismo renderPass)
    if (error) return;
...
{
    PipelineUtils::createImage(device,width,height,colorFormat,VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
        VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
        VK_IMAGE_USAGE_SAMPLED_BIT, multiSamplerPower,VK_IMAGE_ASPECT_COLOR_BIT, colorImage,colorImageMemory,colorImageView);
    if (colorSampler ==VK_NULL_HANDLE) {
        PipelineUtils::createSampler(device->device,colorSampler,VK_FILTER_LINEAR,VK_FILTER_LINEAR,VK_BORDER_COLOR_INT_OPAQUE_BLACK);
    }
}
void FrameBufferObject::createDepthResources()
{
    ...
    PipelineUtils::createImage(device,width,height,depthFormat,usage,multiSamplerPower,VK_IMAGE_ASPECT_DEPTH_BIT,depthImage,depthImageMemory,depthImageView);
    if (depthSamplerEnabled && depthSampler == VK_NULL_HANDLE)
    {
        PipelineUtils::createSampler(device->device,depthSampler,VK_FILTER_LINEAR,VK_FILTER_LINEAR,VK_BORDER_COLOR_INT_OPAQUE_WHITE);
    }

}
void FrameBufferObject::createResources()
{
...
void FrameBufferObject::destroyResources()
{
...
void FrameBufferObject::createRenderPass()
{
    ...
    auto samples = static_cast<VkSampleCountFlagBits>(1 << multiSamplerPower);
