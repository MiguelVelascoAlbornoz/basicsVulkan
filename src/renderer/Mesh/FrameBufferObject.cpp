...
bool createImage(VulkanDevice* device,
...
)
{
    ...
    imageInfo.samples       = static_cast<VkSampleCountFlagBits>(std::pow(2,samples));;
...
{
    createImage(device,width,height,colorFormat,VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
    ...
    if (colorSampler ==VK_NULL_HANDLE) {
        createSampler(device->device,colorSampler,VK_FILTER_LINEAR,VK_FILTER_LINEAR,VK_BORDER_COLOR_INT_OPAQUE_BLACK);
    }
}
